...
static int Check_DataRecv(int pos);
int gClientNum;
int firstflag = 0;
...
void Wii_Button(int pos)
{
	...
	if(dataServer.wiimote.keys.up==1){ //押された時
		if (pos==0){
			dataStruct.chara1.x-=3;
		}else{
			dataStruct.chara2.x-=3;
		}		
        }
        //右に移動(十字キーの下ボタン）
		...
		if (pos==0){
			dataStruct.chara1.y-=3;
		}else{
			dataStruct.chara2.y-=3;
		}
        }
        //下に移動(十字キーの左ボタン）
...
/*****************************************************************
関数名 : Check_DataRecv
機能   : 両方のクライアントからデータがきているか
引数   : int pos クライアントID
出力   : 揃えば真、揃わなければ偽
******************************************************************/
static int Check_DataRecv(int pos)
{
	static int s0=0,s1=0;
	if ( (s0 == 1) && (s1 == 1) ){
		s0 = 0;
		s0 = 0;
		return 1;
